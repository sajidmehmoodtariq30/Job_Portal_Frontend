...
const AdminChatRoom = ({ jobId }) => {
  ...
  const getAdminInfo = () => {
    ...
    return {
      ...
      id: adminInfo.id || adminInfo.uuid || '',
      type: 'admin' // Since this is the admin side component
    };
  };
  ...
  const fetchUnreadCount = async () => {
    ...
    try {
      ...
      const response = await axios.get(API_ENDPOINTS.CHAT.GET_UNREAD(jobId, admin.type));
  ...
  return (
    <Card className="w-full h-[550px] flex flex-col">      <CardHeader className="pb-3 bg-blue-50">
        <div className="flex justify-between items-center">
          <CardTitle className="text-lg font-semibold flex items-center">
            <MessageSquare className="w-5 h-5 mr-2 text-blue-600" />
            Admin Chat {jobId && <span className="text-sm font-normal text-gray-500 ml-1">#{jobId.substring(0, 8)}</span>}
          </CardTitle>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Admin messages will appear on the left, client messages on the right
        </p>
      </CardHeader>
      ...
      <CardContent className="flex-grow overflow-y-auto pb-2">
        {loading && messages.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {messages.map((msg, idx) => {              const isAdmin = msg.senderType === 'admin';
              const isClient = msg.senderType === 'client';
              const admin = getAdminInfo();
              const isCurrentUser = admin.type === msg.senderType;
              ...
              return (
                <React.Fragment key={msg.id}>
                  ...
                  {/* Admin messages on left, client messages on right */}
                  <div className={`flex ${isClient ? 'justify-end' : 'justify-start'}`}>
                    <div className={`flex max-w-[80%] ${isClient ? 'flex-row-reverse' : 'flex-row'} items-start gap-2`}>                      <Avatar className={`h-8 w-8 flex items-center justify-center ${isAdmin ? 'bg-blue-600 text-white' : 'bg-green-600 text-white'}`}>
